...
func (ui *GomuksTUI) loadProxyConfig() {
	switch ui.Config.ConnectionType {
	case "Tor":
		ui.ProxyConfig = widget.ProxyConfig{
	...
	case "Custom":
		pt := widget.ProxyTypeHTTP
		if ui.Config.ProxyType == "socks5" {
			pt = widget.ProxyTypeSOCKS5
		}
		ui.ProxyConfig = widget.ProxyConfig{
			ProxyType: pt,
			Host:      ui.Config.ProxyHost,
			Port:      ui.Config.ProxyPort,
			Username:  ui.Config.ProxyUsername,
			Password:  ui.Config.ProxyPassword,
		}
	}
}
