...
// The internal tools (split across the agent_tools_* translation units;
// declared here so the TUs can call each other).
std::string baseEndpoint(const Config& cfg);
std::string toolSchemasJson();
std::string truncateOut(std::string s, size_t bytes = 20000, size_t lines = 2000);
...
void buildOpenAiMessages(const std::vector<Message>& history, const std::string& sys,
                         nlohmann::json& out);
void buildAnthropicMessages(const std::vector<Message>& history, const std::string& sys,
                            nlohmann::json& out);
nlohmann::json builtinToolsParam();
nlohmann::json openAiToolsParam(const nlohmann::json& src);
int contextSizeFor(const std::string& model);
Message parseAnthropicResponse(const nlohmann::json& resp);
