...
void Database::migrate() {
    ...
    if (!tableHasColumn(_db, "rooms", "version")) {
        exec("ALTER TABLE rooms ADD COLUMN version INTEGER DEFAULT 0");
        exec("ALTER TABLE rooms ADD COLUMN creator TEXT DEFAULT ''");
    }
}
...
bool Database::clearRoom(const std::string& room_id) {
    if (!_db) return false;
    sqlite3_exec(_db, "DELETE FROM events WHERE room_id = ?", nullptr, nullptr, nullptr);
    // Re-prepare with the bound value.
    sqlite3_stmt* stmt = nullptr;
    const char* sql = "DELETE FROM events WHERE room_id = ?;";
    ...
    const char* rsql = "DELETE FROM rooms WHERE room_id = ?;";
    if (sqlite3_prepare_v2(_db, rsql, -1, &rstmt, nullptr) != SQLITE_OK) return false;
    sqlite3_bind_text(rstmt, 1, room_id.c_str(), -1, SQLITE_TRANSIENT);
    sqlite3_step(rstmt);
...
std::vector<std::string> Database::invitedRoomIds(const std::string& userId) const {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (rid) result.emplace_back(rid);
    }
    sqlite3_finalize(stmt);
    return result;
}
